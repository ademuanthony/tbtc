...
export const saveMint = async (mint: Mint) => {
  const updatedWalletString = JSON.stringify(mint, null, 2)
  await fs.writeFileSync(`./.data/${mint.label}.json`, updatedWalletString)
}

export const loadMint = async (label: string) => {
  const filePath = `./.data/${label}.json`
  const walletString = fs.readFileSync(filePath, 'utf-8')
  const project = JSON.parse(walletString) as Mint
